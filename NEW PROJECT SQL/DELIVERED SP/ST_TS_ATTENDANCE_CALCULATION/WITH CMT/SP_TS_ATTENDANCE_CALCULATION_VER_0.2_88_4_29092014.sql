...
BEGIN
		...
		IF(@HALFDAYLEAVE!=0)THEN
			WHILE(I<=@HALFDAYLEAVE)DO
				...
				IF(@AM=1 AND @PM=2)THEN
					...
					(SELECT UARD_DATE,"MORNING PRESENT","AFTERNOON ABSENT" FROM USER_ADMIN_REPORT_DETAILS WHERE ULD_ID=',ULDID,' AND UARD_DATE BETWEEN ','"',MONTH_STARTDATE,'"',' AND ','"',MONTH_ENDDATE,'"',' AND UARD_ATTENDANCE=4 AND UARD_AM_SESSION IN (1,2) AND UARD_PM_SESSION IN (1,2) LIMIT ',N,',1)'));
					PREPARE INSERT_TEMP_ATTENDANCE_STMT FROM @INSERT_TEMP_ATTENDANCE;
					EXECUTE INSERT_TEMP_ATTENDANCE_STMT;
				ELSEIF(@AM=2 AND @PM=1)THEN
					SET @INSERT_TEMP_ATTENDANCE=(SELECT CONCAT('INSERT INTO ',TEMP_ATTENDANCE,'(REPORT_DATE,PRESENT,ABSENT)
					(SELECT UARD_DATE,"AFTERNOON PRESENT","MORNING ABSENT" FROM USER_ADMIN_REPORT_DETAILS WHERE ULD_ID=',ULDID,' AND UARD_DATE BETWEEN ','"',MONTH_STARTDATE,'"',' AND ','"',MONTH_ENDDATE,'"',' AND UARD_ATTENDANCE=4 AND UARD_AM_SESSION IN (1,2) AND UARD_PM_SESSION IN (1,2) LIMIT ',N,',1)'));
					PREPARE INSERT_TEMP_ATTENDANCE_STMT FROM @INSERT_TEMP_ATTENDANCE;
					EXECUTE INSERT_TEMP_ATTENDANCE_STMT;
				END IF;
				SET N=N+1;
		...
		IF(@HALFDAYOD!=0)THEN
			WHILE(I<=@HALFDAYOD)DO
				...
				IF(@AM_OD=1 AND @PM_OD=2)THEN
					...
					(SELECT UARD_DATE,"MORNING PRESENT","AFTERNOON ONDUTY" FROM USER_ADMIN_REPORT_DETAILS WHERE ULD_ID=',ULDID,' AND UARD_DATE BETWEEN ','"',MONTH_STARTDATE,'"',' AND ','"',MONTH_ENDDATE,'"',' AND UARD_ATTENDANCE=8 AND UARD_AM_SESSION IN (1,2) AND UARD_PM_SESSION IN (1,2) LIMIT ',N,',1)'));
					PREPARE INSERT_TEMP_ATTENDANCE_STMT FROM @INSERT_TEMP_ATTENDANCE;
					EXECUTE INSERT_TEMP_ATTENDANCE_STMT;
				ELSEIF(@AM_OD=2 AND @PM_OD=1)THEN
					SET @INSERT_TEMP_ATTENDANCE=(SELECT CONCAT('INSERT INTO ',TEMP_ATTENDANCE,'(REPORT_DATE,PRESENT,ONDUTY)
					(SELECT UARD_DATE,"AFTERNOON PRESENT","MORNING ONDUTY" FROM USER_ADMIN_REPORT_DETAILS WHERE ULD_ID=',ULDID,' AND UARD_DATE BETWEEN ','"',MONTH_STARTDATE,'"',' AND ','"',MONTH_ENDDATE,'"',' AND UARD_ATTENDANCE=8 AND UARD_AM_SESSION IN (1,2) AND UARD_PM_SESSION IN (1,2) LIMIT ',N,',1)'));
					PREPARE INSERT_TEMP_ATTENDANCE_STMT FROM @INSERT_TEMP_ATTENDANCE;
					EXECUTE INSERT_TEMP_ATTENDANCE_STMT;
				END IF;
				SET N=N+1;
	...
	IF (LOGIN_ID IS NULL) THEN
		...
		WHILE(US_MIN_ID<=US_MAX_ID)DO
			...
			(','"',LOGIN_ID,'"',',',TOTAL_DAYS,',',T_PRESENT,',',T_ABSENT,',',T_ONDUTY,',',T_PERMISSION,')'));
			PREPARE INSERT_TEMP_ATTENDANCE_CALCULATION_STMT FROM @INSERT_TEMP_ATTENDANCE_CALCULATION;
			EXECUTE INSERT_TEMP_ATTENDANCE_CALCULATION_STMT;
...
END;
/*
CALL SP_TS_ATTENDANCE_CALCULATION('JULY-2014','','dhandapani.sattanathan@ssomens.com',@TEMP_ATTENDANCE_CALCULATION,@TOTAL_DAYS);
SELECT @TOTAL_DAYS;
*/