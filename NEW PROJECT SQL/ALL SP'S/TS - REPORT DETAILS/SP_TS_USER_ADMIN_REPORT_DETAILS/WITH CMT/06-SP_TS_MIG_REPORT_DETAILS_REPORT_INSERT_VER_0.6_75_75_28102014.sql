-- VERSION 0.6 STARTDATE:28/10/2014 ENDDATE:28/10/2014 ISSUE NO:75 COMMENT NO:75 DESC:UPDATE PROJECT ID. DONE BY :RAJA
-- VERSION 0.5 STARTDATE:21/10/2014 ENDDATE:21/10/2014 ISSUE NO:75 COMMENT NO:47 DESC:REPORT CHANGED AS REPORTS. DONE BY :RAJA
-- VERSION 0.4 STARTDATE:13/10/2014 ENDDATE:13/10/2014 ISSUE NO:78 COMMENT NO:155 DESC:COUNT THE REJECTION RATE FOR SOURCE AND DESTI. DONE BY :RAJA
...
-- VERSION 0.2 STARTDATE:12/09/2014 ENDDATE:12/09/2014 ISSUE NO:78 COMMENT NO:67 DESC:MODIFIED THE USER ADMIN REPORT TABLE FOR THE NEW MIG REPORT. DONE BY :RAJA
-- VERSION 0.1 STARTDATE:25/08/2014 ENDDATE:26/08/2014 ISSUE NO:78 COMMENT NO:33 DESC:SP FOR INSERT ALL THE REPORTS MIGRATION GET SOURCE TO DESTINATION DYNAMICALLY. DONE BY :RAJA
...
BEGIN
	...
	IF (PARTNO=6) THEN
		...
		SET @UPDATETIMESTAMP=(SELECT CONCAT('UPDATE ',DESTINATIONSCHEMA,'.TEMP_USER_ADMIN_REPORT_DETAILS SET UARD_TIMESTAMP=(SELECT CONVERT_TZ(UARD_TIMESTAMP, "+05:30","+0:00"))'));
		PREPARE UPDATETIMESTAMPSTMT FROM @UPDATETIMESTAMP;
		EXECUTE UPDATETIMESTAMPSTMT;
		SET @UPDATEPROJECT=(SELECT CONCAT('UPDATE ',DESTINATIONSCHEMA,'.TEMP_USER_ADMIN_REPORT_DETAILS SET UARD_PDID=(SELECT PD_ID FROM ',DESTINATIONSCHEMA,'.PROJECT_DETAILS WHERE PD_PROJECT_NAME="MARKETING"),UARD_TIMESTAMP=UARD_TIMESTAMP WHERE ULD_ID=42 AND UARD_ATTENDANCE NOT IN(6,7)'));
		PREPARE UPDATEPROJECT_STMT FROM @UPDATEPROJECT;
		EXECUTE UPDATEPROJECT_STMT;
		SET @UPDATEPROJECT=(SELECT CONCAT('UPDATE ',DESTINATIONSCHEMA,'.TEMP_USER_ADMIN_REPORT_DETAILS SET UARD_PDID=(SELECT PD_ID FROM ',DESTINATIONSCHEMA,'.PROJECT_DETAILS WHERE PD_PROJECT_NAME="EI CRM/MARKETING"),UARD_TIMESTAMP=UARD_TIMESTAMP WHERE ULD_ID=11 AND UARD_ATTENDANCE NOT IN(6,7)'));
		PREPARE UPDATEPROJECT_STMT FROM @UPDATEPROJECT;
		EXECUTE UPDATEPROJECT_STMT;
		SET @UPDATEPROJECT=(SELECT CONCAT('UPDATE ',DESTINATIONSCHEMA,'.TEMP_USER_ADMIN_REPORT_DETAILS SET UARD_PDID=(SELECT PD_ID FROM ',DESTINATIONSCHEMA,'.PROJECT_DETAILS WHERE PD_PROJECT_NAME="OFFICE MAINTENANCE"),UARD_TIMESTAMP=UARD_TIMESTAMP WHERE ULD_ID IN (41,66) AND UARD_ATTENDANCE NOT IN(6,7)'));
		PREPARE UPDATEPROJECT_STMT FROM @UPDATEPROJECT;
		EXECUTE UPDATEPROJECT_STMT;
		SET @TRUNCATE_USER_ADMIN_REPORT_DETAILS=(SELECT CONCAT('TRUNCATE ',DESTINATIONSCHEMA,'.USER_ADMIN_REPORT_DETAILS'));
		PREPARE TRUNCATE_USER_ADMIN_REPORT_DETAILS_STMT FROM @TRUNCATE_USER_ADMIN_REPORT_DETAILS;
		EXECUTE TRUNCATE_USER_ADMIN_REPORT_DETAILS_STMT;
...
END;
/*
CALL SP_TS_MIG_REPORT_DETAILS_REPORT_INSERT('TS_SOURCE','TS_DEST',1,1,1500,'dhandapani.sattanathan@ssomens.com');
CALL SP_TS_MIG_REPORT_DETAILS_REPORT_INSERT('TS_SOURCE','TS_DEST',2,1501,3000,'dhandapani.sattanathan@ssomens.com');
CALL SP_TS_MIG_REPORT_DETAILS_REPORT_INSERT('TS_SOURCE','TS_DEST',3,3001,4500,'dhandapani.sattanathan@ssomens.com');
CALL SP_TS_MIG_REPORT_DETAILS_REPORT_INSERT('TS_SOURCE','TS_DEST',4,4501,6000,'dhandapani.sattanathan@ssomens.com');
CALL SP_TS_MIG_REPORT_DETAILS_REPORT_INSERT('TS_SOURCE','TS_DEST',5,6001,7500,'dhandapani.sattanathan@ssomens.com');
CALL SP_TS_MIG_REPORT_DETAILS_REPORT_INSERT('TS_SOURCE','TS_DEST',6,7501,NULL,'dhandapani.sattanathan@ssomens.com');
*/