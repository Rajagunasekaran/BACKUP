...
BEGIN
	...
	(SELECT ULD.ULD_ID,ULD.EMPLOYEE_NAME,URCN.URC_ID FROM VW_TS_ALL_EMPLOYEE_DETAILS ULD,USER_ACCESS UA, VW_USER_ACCESS_MAXRECVER V,USER_RIGHTS_CONFIGURATION URCN,ROLE_CREATION RC WHERE ULD.ULD_ID=UA.ULD_ID 
	...
	IF(COUNT_REPORT!=0)THEN
		...
		WHILE(TAU_MIN_ID <= TAU_MAX_ID)DO  
			SET @INSERT_USERADMINREPORTDETAILS=(SELECT CONCAT('INSERT INTO USER_ADMIN_REPORT_DETAILS(UARD_REASON,UARD_DATE,UARD_ATTENDANCE,UARD_AM_SESSION,UARD_PM_SESSION,URC_ID,ULD_ID,UARD_BANDWIDTH,CIORL_ID,UARD_USERSTAMP_ID,ABSENT_FLAG) VALUES 
			(("REPORT MISSED TO ENTER"),(CURDATE()),(SELECT DISTINCT AC_ID FROM ATTENDANCE_CONFIGURATION WHERE AC_DATA="0" AND CGN_ID=5),
			...
			(SELECT DISTINCT ULD_ID FROM ',TEMP_ABSENT_USER,' WHERE ID=',TAU_MIN_ID,'),(0),(1),(',ULDID,'),("X"))'));      
