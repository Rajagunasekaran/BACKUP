-- VERSION 0.2 STARTDATE:23/10/2014 ENDDATE:23/10/2014 ISSUE NO:76 COMMENT NO:18 DESC:CREATED EMAIL TEMPLATE TABLES. DONE BY :RAJA
-- VERSION 0.1 STARTDATE:13/10/2014 ENDDATE:13/10/2014 ISSUE NO:78 COMMENT NO:155 DESC:SP FOR CREATE ALL TABLE. DONE BY :RAJA
...
BEGIN 
	...
	FOREIGN KEY(UA_EMP_TYPE) REFERENCES ',DESTINATIONSCHEMA,'.USER_RIGHTS_CONFIGURATION (URC_ID),
	FOREIGN KEY(ULD_ID) REFERENCES ',DESTINATIONSCHEMA,'.USER_LOGIN_DETAILS (ULD_ID))'));
	...
	PRIMARY KEY(ET_ID),
	UNIQUE(ET_EMAIL_SCRIPT))'));
	PREPARE CREATE_EMAIL_TEMPLATE_STMT FROM @CREATE_EMAIL_TEMPLATE;
	EXECUTE CREATE_EMAIL_TEMPLATE_STMT;
	-- EMAIL TEMPLATE DETAILS
	SET @DROP_EMAIL_TEMPLATE=(SELECT CONCAT('DROP TABLE IF EXISTS ',DESTINATIONSCHEMA,'.EMAIL_TEMPLATE_DETAILS'));
	PREPARE DROP_EMAIL_TEMPLATE_STMT FROM @DROP_EMAIL_TEMPLATE;
	EXECUTE DROP_EMAIL_TEMPLATE_STMT;
	SET @CREATE_EMAIL_TEMPLATE=(SELECT CONCAT('CREATE TABLE ',DESTINATIONSCHEMA,'.EMAIL_TEMPLATE_DETAILS(
