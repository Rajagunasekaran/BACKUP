...
{
    $selectquery=mysqli_query($con,"SELECT A.*,B.LJS_STATUS,C.LVS_STATUS,D.CLD_CONTRACT_NO FROM LMC_MAINTAIN_LOCATION_DETAILS A, LMC_JOB_STATUS B, LMC_VERIFICATION_STATUS C, LMC_CONTRACT_DETAILS D WHERE A.LJS_ID=B.LJS_ID AND A.LVS_ID = C.LVS_ID AND A.CLD_ID=D.CLD_ID ORDER BY A.LML_REF_NO ASC");
    $arrayvalues=[];
