...
<script>
    ...
    //CANCEL BUTTON CLICK EVENT
        $('#CC_btn_cancel').click(function(){
            ...
            $('#CC_btn').hide();
            $('#CC_btn_additem').show();
            $("#contractcreation").find('input:text, input:password, input:file,textarea').val('');
            $("#contractcreation").find('select').val('SELECT');
            $("#contractcreation").find('input:radio, input:checkbox').removeAttr('checked').removeAttr('selected');
        });
        var result=[];
                ...
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    ...
                    if (result.length>0) {
                        ...
                        for (var i = 0; i <result.length; i++) {
                            ...
                            CC_UPD_table_header += '<tr id='+rowid+' ><td style="text-align:center;"><div class="col-lg-1"><span style="display: block;color:green" class="glyphicon glyphicon-edit  edit" id="edit_'+rowid+'"></span></div></td><td nowrap style="text-align:center;">' +contractno+ '</td><td style="text-align:center;">' +customername+ '</td><td style="text-align:center;"> '+address+ '</td><td  style="text-align:center;">' +contactperson+ '</td><td nowrap>' +createddate+ '</td><td nowrap>' +enddate+ '</td><td nowrap>' +extendedate+ '</td><td>' +teamname+ '</td><td nowrap style="text-align:center;">' +status+ '</td><td style="text-align:center;">' +userstamp+ '</td><td style="text-align:center;" nowrap>' +timestamp+ '</td></tr>';
                        }
