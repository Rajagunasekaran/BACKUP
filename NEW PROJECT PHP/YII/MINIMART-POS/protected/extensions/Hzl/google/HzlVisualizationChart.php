...
class HzlVisualizationChart extends CWidget {
    ...
    public function registerClientScript() {
        ...
        if(!empty($this->url))
        {
            $tmp = 'var jsonData = $.ajax({
                                              url: "'. $this->url . '",
                                              dataType:"json",
                                              async: false
                                              }).responseText;
                        jsonData = $.parseJSON(jsonData);
                        jsonData=formatMstaskforchartdata(jsonData);
                        var data = google.visualization.arrayToDataTable(jsonData,' . (($this->firstRow)?'true':'false') . ');                     
                        ';
        }
        else
        {
           $jsData = CJavaScript::jsonEncode($this->data);
           $tmp = 'var data = google.visualization.arrayToDataTable(' . $jsData . ',' . (($this->firstRow)?'true':'false') . ');';
        }        
        
        $script .=  $tmp . ' var options = ' . $jsOptions . ';
