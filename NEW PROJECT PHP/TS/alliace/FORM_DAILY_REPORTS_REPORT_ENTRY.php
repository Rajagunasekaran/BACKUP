...
$(document).ready(function(){
    ...
    $(document).on("change",'.fileextensionchk', function (){
        for(var i=0;i<25;i++)
        {
            ...
            {
                $(document).doValidation({rule:'messagebox',prop:{msgtitle:"ADMIN REPORT ENTRY",msgcontent:err_msg[12],position:{top:150,left:500}}});
...
//    $(".amountonly").doValidation({rule:'numbersonly',prop:{realpart:4,imaginary:2}});
    ...
    $(document).on('change','#URE_tb_date',function(){
        ...
        var option="DATE";
        xmlhttp.open("GET","DB_DAILY_REPORTS_REPORT_ENTRY.php?date_change="+reportdate+"&option="+option);
        xmlhttp.send();
    });
...
//                        URE_tble_bandwidth();
                        ...
                        $('#URE_lbl_checkmsg').hide();
                        $('#URE_tble_paint').show();
                        $("#URE_tble_attachment").show();
                    }
                }

            }
            var option="PRESENT";
            xmlhttp.open("POST","DB_DAILY_REPORTS_REPORT_ENTRY.php?option="+option+"&reportdate="+reportdate);
            xmlhttp.send();
...
//            var URE_bndtxt = $("#URE_tb_band").val();
    ...
    $(document).on('click','#URE_btn_submit',function(){
        ...
        // Send Request
        request = $.ajax({
            url: "DB_DAILY_REPORTS_REPORT_ENTRY.php",
            type: "POST",
            data: formElement+"&option="+option+"&string="+imgData,
            success: function(response){
                var msg_alert=response;
                $('.preloader', window.parent.document).hide();
                if(msg_alert==1)
                {
                    $(document).doValidation({rule:'messagebox',prop:{msgtitle:"REPORT ENTRY",msgcontent:err_msg[0],position:{top:150,left:500}}});
                    UARD_clear();
                    $('#URE_tb_date').val('');
                }
                if(msg_alert==0)
                {
                    $(document).doValidation({rule:'messagebox',prop:{msgtitle:"REPORT ENTRY",msgcontent:err_msg[4],position:{top:150,left:500}}});
//                    UARD_clear();
//                    $('#URE_tb_date').val('');
                }
                if(msg_alert!=0 && msg_alert!=1)
                {
                    $(document).doValidation({rule:'messagebox',prop:{msgtitle:"REPORT ENTRY",msgcontent:msg_alert,position:{top:150,left:500}}});
//                    UARD_clear();
//                    $('#URE_tb_date').val('');
                }
            }
...
//                        $('#URE_tble_mulpaint').hide();
            ...
            xmlhttp.open("GET","DB_DAILY_REPORTS_REPORT_ENTRY.php?option="+option+"&fromdate="+fromdate+"&todate="+todate,true);
            xmlhttp.send();
        }
    });
});
...
<div class="wrapper">
    ...
    <div class="title" id="fhead" ><div style="padding-left:500px; text-align:left;""><h3><p>REPORT ENTRY</p></h3></div></div>
