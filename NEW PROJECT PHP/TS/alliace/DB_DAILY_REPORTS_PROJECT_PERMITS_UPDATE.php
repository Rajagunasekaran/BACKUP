...
where UARD_DATE BETWEEN '$startdate' AND '$enddate' and UARD.ULD_ID='$ure_uld_id' ORDER BY UARD.UARD_DATE ");
        ...
        while($row=mysqli_fetch_array($date)){
            ...
            if($ure_fileid!=null || $ure_fileid!=''){
                ...
                $path = $upload_dir.$ure_fileid.".png";
...
//            $location=$row['CIORL_LOCATION'];
    ...
    if($_POST['choice']=='PROJECT PERMITS UPDATE')
    {
...
// for absent radio button
        ...
        if($fileflg!=0){
            $result = $con->query("CALL SP_TS_DAILY_REPORT_SEARCH_UPDATE($id,'$report','$reason','$finaldate',$ADM_urc_id,'$login_id','$perm_time','$ADM_attendance','$projectid','$uard_morning_session','$uard_afternoon_session','$USERSTAMP','$flag_absent','$filename','$upload_filename',@success_flag)");
            if(!$result){
                unlink($file);
        ...
        {
            $upload_dir = $dir.$parentfolder.DIRECTORY_SEPARATOR.$userfolderid.DIRECTORY_SEPARATOR;
            $deltpath = $upload_dir.$old_fileid.".png";
            unlink($deltpath);
        }
        if($flag==1)
        {
...
//                $mail->SMTPDebug = 1;
                ...
                $mail->AddAddress("safiyullah.mohideen@ssomens.com");
                if(!$mail->Send()){
                    $flag=0;
                }else{
                    $flag=1;
    ...
    if($_REQUEST['option']=='DELETE')
    {
        ...
        $fid= mysqli_query($con,"SELECT UARD_FILE_ID,ULD_ID FROM USER_ADMIN_REPORT_DETAILS WHERE UARD_ID=$id");
        ...
        if($flag==1 && ($file_id!=null || $file_id!=''))
        {
            $userfolderid=get_emp_folderid($ure_uld_id);
            $upload_dir = $dir.$parentfolder.DIRECTORY_SEPARATOR.$userfolderid.DIRECTORY_SEPARATOR;
            $deletepath = $upload_dir.$file_id.".png";
            unlink($deletepath);
        }
        echo $flag;
    }
