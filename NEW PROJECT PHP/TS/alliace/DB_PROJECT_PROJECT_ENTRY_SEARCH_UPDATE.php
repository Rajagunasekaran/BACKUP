...
if(isset($_REQUEST))
{
    ...
    $image_folder='ATS_DOCUMENT/';
    $uploaded = false;
    $filename='';
    if($_REQUEST['option']=='SAVE')
    {
        $uploadcount=$_REQUEST['upload'];
        for($x=0;$x<$uploadcount;$x++)
        {
            if($x==0)
            {
                $filename=$_FILES['upload_filename'.$x]['name'];
            }
            else
            {
                $filename=$filename.'/'.$_FILES['upload_filename'.$x]['name'];
            }
            move_uploaded_file($_FILES['upload_filename'.$x]['tmp_name'],$image_folder.$_FILES['upload_filename'.$x]['name']);
        }
        ...
        if($_REQUEST['PE_tb_id']=='')
        {
            $project_status=$_REQUEST['PE_tb_status'];
            $projectid=0;
            $psid=0;
            $result = $con->query("CALL SP_TS_PROJECT_DETAILS_INSERT_UPDATE('$projectid','$project_name','$project_des','$psid','$project_status','$project_sdate','$project_edate','$filename','$USERSTAMP','INSERT',@success_flag)");         if(!$result) die("CALL failed: (" . $con->errno . ") " . $con->error);
        ...
        {
            $project_status=$_REQUEST['PE_tb_statusupdate'];
            $id=$_REQUEST['PE_tb_id'];
            $split_id=explode("_",$id);
            $projectid=$split_id[1];
            $psid=$split_id[0];
            $QUERY= "CALL SP_TS_PROJECT_DETAILS_INSERT_UPDATE('$projectid','$project_name','$project_des','$psid','$project_status','$project_sdate','$project_edate','$filename','$USERSTAMP','UPDATE',@success_flag)";
    ...
    else if($_REQUEST['option']=='AUTO')
    {
        ...
        $erro_msg=get_error_msg('62,63,79,80,81,83,99,133');
...
function showData($data,$con){
    $sql = "SELECT PC.PC_DATA,PC.PC_ID,PS.PS_ID,PS.PS_REC_VER,DATE_FORMAT(PS.PS_END_DATE,'%d-%m-%Y')as PS_END_DATE,DATE_FORMAT(PS.PS_START_DATE,'%d-%m-%Y') as PS_START_DATE,PD.PD_ID,PD.PD_PROJECT_NAME,PS.PS_PROJECT_DESCRIPTION,DATE_FORMAT(CONVERT_TZ(PS.PS_TIMESTAMP,'+00:00','+08:00'), '%d-%m-%Y %T') as TIMESTAMP,ULD.ULD_LOGINID as  ULD_USERSTAMP,PS.PS_FILE_NAME FROM PROJECT_DETAILS PD JOIN PROJECT_STATUS PS on PD.PD_ID = PS.PD_ID JOIN PROJECT_CONFIGURATION PC on PS.PC_ID = PC.PC_ID JOIN USER_LOGIN_DETAILS ULD on PS.ULD_ID=ULD.ULD_ID ORDER BY PD_PROJECT_NAME";
    $data = $con->query($sql);
    $str='<thead  bgcolor="#6495ed" style="color:white"><tr class="head"><th width=50> </th><th  width=200>PROJECT NAME</th><th width=500 >PROJECT DESCRIPTION</th><th width=10>REC VER</th><th width=30>STATUS</th><th width=80 class="uk-date-column">START DATE</th><th width=50 class="uk-date-column">END DATE</th><th width=100 class="uk-date-column">FILE NAME</th><th style="min-width:70px;">USERSTAMP</th><th style="min-width:100px;" nowrap class="uk-timestp-column">TIMESTAMP</th></tr></thead><tbody>';
//    $str='';
    if($data->num_rows>0){
        while( $row = $data->fetch_array(MYSQLI_ASSOC)){
            $str.="<tr id='".$row['PS_ID'].'_'.$row['PD_ID']."'><td width=50><input type='button'  id='editbtn' class='ajaxedit' style='background: url(image/edit.png);height:30px;width:30px;' title='Edit'/></td><td width=200 style='font-weight:bold;'>".$row['PD_PROJECT_NAME']." </td><td width=500>".$row['PS_PROJECT_DESCRIPTION']."</td><td width=10 >".$row['PS_REC_VER']."</td><td width=30>".$row['PC_DATA']."</td><td width=80 nowrap>".$row['PS_START_DATE']."</td><td width=90 nowrap>".$row['PS_END_DATE']."</td><td width=100 nowrap>".$row['PS_FILE_NAME']."</td><td width=70>".$row['ULD_USERSTAMP']."</td><td width=120 nowrap>".$row['TIMESTAMP']."</td></tr>";
        }
        echo $str;
    }
    else{
