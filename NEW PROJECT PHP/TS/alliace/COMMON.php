...
function get_permission(){
    global $con;
    $permission_result = mysqli_query($con,"SELECT AC_DATA FROM ATTENDANCE_CONFIGURATION WHERE CGN_ID=4");
...
function get_docfolder_id(){
    global $con;
    $docid=mysqli_query($con,"SELECT URC_DATA FROM USER_RIGHTS_CONFIGURATION WHERE URC_ID=9 AND CGN_ID=9");
...
function get_parentfolder_id(){
    global $con;
    $parentid=mysqli_query($con,"SELECT URC_DATA FROM USER_RIGHTS_CONFIGURATION WHERE URC_ID=8 AND CGN_ID=8");
...
//    echo $USERSTAMP;
    ...
    $error='3,4,5,6,7,8,16,17,18,67,115,132,133';
...
if($_REQUEST["option"]=="user_search_update"){
    ...
    $error='3,4,5,6,7,8,16,17,18,67,83,98,132,133,141,142';
    $error_array=get_error_msg($error);
    $min_date=get_joindate($ure_uld_id);
...
//    $get_project_array=get_project();
    $error='3,4,5,6,7,8,16,17,18,67,83,98,109,110,132,133';
    $error_array=get_error_msg($error);
