...
function qt_custom_breadcrumbs() {
	...
	$blogLink =  get_bloginfo('url')."/community/newsroom";
	...
	if (is_front_page()) {
		...
		echo '<div id="crumbs" class="breadcrumbs"><a href="' . $homeLink . '">'.$home. '</a> ' . $delimiter . '<a href="' . $blogLink . '">'.' '. $before.' '.$blog.''.$after.'</a></div>';
		...
		if ( is_category() ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				if ($showCurrent == 1) {
					...
					if($post_type!='event_detail') {
						echo ' ' . $delimiter . ' ' . $before . get_the_title() . $after;
					} else {
						$title = get_field('event_title',$post->ID);
